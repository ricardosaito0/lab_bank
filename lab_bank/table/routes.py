from flask import render_template, flash, request, redirect, url_for, abort, Blueprint, current_app, send_from_directory
...
#         # Handle file uploads
#         def save_uploaded_file(file_field, folder_name):
#             if file_field and file_field.filename:
#                 timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
#                 original_filename = secure_filename(file_field.filename)
#                 filename = f"{timestamp}_{original_filename}"
...
#         excel_file_path = save_uploaded_file(form.excel_file.data, 'excel_files')
#         parameters_file_path = save_uploaded_file(form.parameters_file.data, 'parameters_files')
#         signals_file_path = save_uploaded_file(form.signals_file.data, 'signals_files')

#         if excel_file_path:
#             flash('Arquivo Excel salvo com sucesso', 'success')
#         else:
#             flash('Nenhum arquivo Excel selecionado.', 'warning')

#         if parameters_file_path:
#             flash('Arquivo de parâmetros salvo com sucesso', 'success')
#         else:
#             flash('Nenhum arquivo de parâmetros selecionado.', 'warning')

#         if signals_file_path:
#             flash('Arquivo de sinais salvo com sucesso', 'success')
#         else:
#             flash('Nenhum arquivo de sinais selecionado.', 'warning')
...
#                 flexivent=flexivent,
#                 excel_file_path=excel_file_path,
#                 parameters_file_path=parameters_file_path,  # Add parameters file path
#                 signals_file_path=signals_file_path,        # Add signals file path
#                 observations=form.observations.data if form.observations.data else ''
#             )
#             db.session.add(subject)
#             db.session.commit()
...
def insert_data():
    ...
    # Initialize current_files with None values
    current_files = {
        'excel_file': None,
    ...
    # Pass current_files and subject to the template
    return render_template('insert_data.html', title='Inserção de dados', legend='Inserção de dados', form=form, current_files=current_files, subject=subject)
...
def display_table():
    ...
    for subject in subjects:
        ...
        excel_download_link = None
        parameters_file_link = None
        signals_file_link = None

        if subject.excel_file_path:
            excel_filename = os.path.basename(subject.excel_file_path)  # Extract only the file name
            excel_file_link = url_for('table.display_excel_file', filename=excel_filename)
            excel_download_link = url_for('table.download_excel_file', filename=excel_filename)  # Download link

        if subject.parameters_file_path:
            parameters_filename = os.path.basename(subject.parameters_file_path)  # Extract only the file name
            parameters_file_link = url_for('table.download_parameters_file', filename=parameters_filename)

        if subject.signals_file_path:
            signals_filename = os.path.basename(subject.signals_file_path)  # Extract only the file name
            signals_file_link = url_for('table.download_signals_file', filename=signals_filename)
        ...
        subjects_data.append({
            ...
            'Lavado broncoaoveolar (/ 4 x 10⁴)': subject.bronchoalveolar_lavage,
            'ID do Usuário': subject.user_id,
            'Caminho para a tabela': f'<a href="{excel_file_link}" target="_blank">Ver Arquivo Excel</a> | <a href="{excel_download_link}" target="_blank">Baixar Arquivo Excel</a>' if excel_file_link else 'Nenhum arquivo',
            'Caminho para parâmetros': f'<a href="{parameters_file_link}" target="_blank">Baixar Arquivo de Parâmetros</a>' if parameters_file_link else 'Nenhum arquivo',
            'Caminho para sinais': f'<a href="{signals_file_link}" target="_blank">Baixar Arquivo de Sinais</a>' if signals_file_link else 'Nenhum arquivo',
            'Observações': subject.observations
        })
...
def update_data(subject_id):
    ...
    if form.validate_on_submit():
        ...
        subject.project = form.project.data if form.project.data else ''
        subject.name_number = form.name_number.data if form.name_number.data else ''
        subject.date = form.date.data  # Date is now properly formatted
        subject.age = form.age.data
        subject.weight = form.weight.data
        subject.naso_anal_length = form.naso_anal_length.data
        subject.bronchoalveolar_lavage = form.bronchoalveolar_lavage.data
        ...
        # Handle file uploads
        def save_uploaded_file(file_field, folder_name, current_file_path):
            if file_field and file_field.filename:
                timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                filename = secure_filename(f"{timestamp}_{file_field.filename}")
                upload_folder = os.path.join(current_app.root_path, 'static', 'uploads', folder_name)
                os.makedirs(upload_folder, exist_ok=True)
                file_path = os.path.join(upload_folder, filename)
        ...
        subject.excel_file_path = save_uploaded_file(form.excel_file.data, 'excel_files', subject.excel_file_path)
        subject.parameters_file_path = save_uploaded_file(form.parameters_file.data, 'parameters_files', subject.parameters_file_path)
        subject.signals_file_path = save_uploaded_file(form.signals_file.data, 'signals_files', subject.signals_file_path)
        ...
        if isinstance(subject.date, datetime):
            form.date.data = subject.date.date()  # Extract only the date part
        elif isinstance(subject.date, str):
            form.date.data = datetime.strptime(subject.date, '%Y-%m-%d').date()  # Convert string to date
        else:
            form.date.data = subject.date  # If it's already a date, just assign it
    ...
    # Get current filenames for display
    current_files = {
        'excel_file': os.path.basename(subject.excel_file_path) if subject.excel_file_path else None,
        'parameters_file': os.path.basename(subject.parameters_file_path) if subject.parameters_file_path else None,
        'signals_file': os.path.basename(subject.signals_file_path) if subject.signals_file_path else None
    }

    return render_template('insert_data.html', title='Editar dados', form=form, legend='Editar dados', current_files=current_files, subject=subject)
...
@table.route('/download/excel/<filename>')
@login_required
def download_excel_file(filename):
    upload_folder = os.path.join(current_app.root_path, 'static', 'uploads', 'excel_files')
    return send_from_directory(upload_folder, filename, as_attachment=True)

@table.route('/download/parameters/<filename>')
@login_required
def download_parameters_file(filename):
    upload_folder = os.path.join(current_app.root_path, 'static', 'uploads', 'parameters_files')
    return send_from_directory(upload_folder, filename, as_attachment=True)

@table.route('/download/signals/<filename>')
@login_required
def download_signals_file(filename):
    upload_folder = os.path.join(current_app.root_path, 'static', 'uploads', 'signals_files')
    return send_from_directory(upload_folder, filename, as_attachment=True)